...
public class QueryQasino {
    // ids
    private static int queryids = 1;
    public final int qid = QueryQasino.queryids++;
    ...
    public QueryQasino(JSONObject json) {
    ...
    public QueryQasino(String query) {
        this.cardinality = 0;
        this.query = query;
        this.realQuery = QueryFactory.create(this.query);
        plan = new QueryPlan(query);
    ...
    public QueryQasino(String query, long card) {
    ...
    public boolean probabilisticIsFinished(double apriori, int numberofpeersseen) {
